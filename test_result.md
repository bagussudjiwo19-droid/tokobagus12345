...
agent_communication:
  ...
  - agent: "main"
    message: "PRINTER BLUETOOTH improvements to verify (frontend + code correctness; REAL Bluetooth hardware is native-only and CANNOT be exercised on web/Expo Go — do NOT mark native-only as failed). Changes: (1) printer.ts adds requestBluetoothPermissions() (PermissionsAndroid: BLUETOOTH_SCAN, BLUETOOTH_CONNECT on API>=31, ACCESS_FINE_LOCATION) and discoverPrinters() (startDiscovery + bonded merge + cancelDiscovery). (2) pengaturan-printer.tsx: button 'Cari Perangkat Bluetooth' (testID printer-scan), 'Scan Ulang' (printer-rescan) after first scan, device rows (printer-device-*) with per-row 'Menghubungkan…' state, choose() now calls connectPrinter then savePrinter and shows green success ('Berhasil terhubung ke ...') or red error ('Gagal terhubung ke printer'), testPrint shows green 'Berhasil mencetak struk' / red 'Gagal mencetak struk'. (3) app.json android.permissions include BLUETOOTH/BLUETOOTH_ADMIN/BLUETOOTH_SCAN/BLUETOOTH_CONNECT/ACCESS_FINE_LOCATION/ACCESS_COARSE_LOCATION. VERIFY (web): opening Riwayat -> printer settings (riwayat-printer-settings) renders without crash; tapping printer-scan on web shows an info toast (native-only) and does NOT crash; the screen shows saved printer + Tes Cetak + Selesai. Also confirm no regression elsewhere. NOTE: actual device discovery/connect/print require a physical Android build."
