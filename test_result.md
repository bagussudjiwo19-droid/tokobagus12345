...
agent_communication:
  ...
  - agent: "main"
    message: "PENGEMBANGAN fitur Bukti Pembayaran → 'Rincian Transaksi' ala aplikasi bayar + Preview Struk 58mm terpisah. App tanpa auth. Base URL dari frontend/.env.\n\nBACKEND: POST /api/ocr/bukti kini mengekstrak 12 field: amount, sender_name, sender_bank, sender_account, recipient, recipient_username, method, ref, txno, product, date, time (tiap {value,confident}). Baca /app/image_testing.md. Buat gambar bukti transfer sintetis berteks: 'SeaBank','Berhasil','Rp51.000','Dari Bagos Sudjiwo SeaBank 901461530896','Ke Shopee Username: sudjiwo14','No. Transaksi 2026082243508309274936490','No. Referensi 15665380490586757524','Metode SeaBank Bayar Instan','Produk Pembayaran Shopee','22 Agu 2026 21:55'. Verifikasi amount=51000 (integer) & field lain terisi confident true. Uji juga gambar tak lengkap → field hilang value=null confident=false. (Sudah diverifikasi manual: semua 12 field terbaca benar.) Sync bukti (s_bukti) tetap ada.\n\nFRONTEND (web):\n(1) Riwayat → ikon kamera (riwayat-bukti-scan) → layar 'Rincian Transaksi' (/baca-bukti).\n(2) Tombol mode: 'Upload Bukti' (bukti-mode-upload) & 'Input Manual' (bukti-mode-manual).\n(3) Isi manual form: Status dropdown (bukti-status → pilih opsi 'BERHASIL'), Nominal (bukti-amount)='51000' (muncul 'Rp 51.000' & kartu atas 'Jumlah Transfer' bukti-amount-big menampilkan Rp 51.000), Dari (bukti-sender-name)='Bagos Sudjiwo', Bank (bukti-sender-bank)='SeaBank', No.Tujuan (bukti-sender-account)='901461530896', Ke (bukti-recipient)='Shopee', Username (bukti-recipient-user)='sudjiwo14', Metode (bukti-method)='SeaBank Bayar Instan', No.Referensi (bukti-ref), No.Transaksi (bukti-txno), Produk (bukti-product), Tanggal (bukti-date), Waktu (bukti-time), Catatan (bukti-note). Kartu 'Rincian Transaksi' di atas ikut ter-update live.\n(4) Tekan 'CETAK STRUK' (bukti-print) → tersimpan + buka '/preview-struk' menampilkan struk 58mm (judul 'BUKTI PEMBAYARAN TOKO BAGUS', STATUS, NOMINAL Rp51.000, DARI/BANK/NO. TUJUAN, KE/USERNAME, METODE/NO. REFERENSI/NO. TRANSAKSI/PRODUK, TANGGAL/WAKTU, TERIMA KASIH). Tombol: preview-share, preview-save-img, preview-print (native-only, cukup pastikan ada & tak crash).\n(5) 'SIMPAN' (bukti-save) → tersimpan ke Riwayat + kembali. Di Riwayat, chip 'Bukti Bayar' (riwayat-filter-bukti) menampilkan kartu (badge BUKTI, metode, Rp51.000). Omzet (riwayat-summary) TIDAK berubah oleh bukti. Tap kartu → /baca-bukti prefill. AKSI LAIN: bukti-share, bukti-save-img (native-only).\n\nNATIVE-ONLY (JANGAN tandai gagal di web): cetak Bluetooth, simpan PNG/PDF, bagikan (PDF), pilih galeri/file, OCR-dari-galeri, Share Target. Fokus: OCR backend 12 field + alur Rincian/form + Preview 58mm + catat terpisah di Riwayat tak masuk omzet."
