...
agent_communication:
  ...
  - agent: "main"
    message: "MIKO RIG 2.5D (frontend-only, kios Cek Harga). New assets assets/miko_rig/*.png (12 frames), new component components/MikoRig.tsx, voice.ts now emits mikoBus speak_start/speak_end, mikoBus.ts adds speak_start/speak_end/miko_state + MikoState type, Miko.tsx exports KIOSK_SAY/KIOSK_TAP, cek-harga.tsx replaces <Miko mode=stage> with <MikoRig> + adds pointing rig on the price result screen and a (currently dormant) rig in the chat panel. PLEASE run FRONTEND-ONLY regression on Cek Harga kiosk (base URL from frontend/.env EXPO_PUBLIC_BACKEND_URL). Admin PIN gate: create-pin-1/create-pin-2 with 1234 then create-pin-save (SecureStore may not persist on web; recreate if prompted). VERIFY no crash / no regression: (1) idle kiosk renders with Miko mascot on pedestal; (2) type 'beras' in cekharga-search-input + press cekharga-search-btn -> product cards list (Pilih Barang) with countdown; (3) tapping a single product (e.g. price Rp 13.700) -> price result card (cekharga-result) shows with a Miko image above it + 'Kembali otomatis dalam Ns' countdown; (4) back arrow (top-left) returns to idle; (5) scanner hidden input still present (no keyboard popup). Native-only (TTS voice, mouth-sync audio) CANNOT be tested on web -> do NOT mark failed. Just confirm rendering + flows + absence of red-screen/console crashes."
