...
- `src/localdb.ts` BARU `safeImportProducts(data)`: bangun set nama (dinormalisasi lowercase+trim+collapse spasi) & set barcode dari produk yg ADA. Normalisasi produk file, kelompokkan induk+anak via rootOf(parent_id chain dalam file). Per grup: nameHit = existNames.has(norm(root.name)); barcodeHit = ada barcode grup ∈ existBarcodes → bila salah satu true LEWATI (reason). Else impor seluruh grup (remap id yg bentrok + remap parent_id anak), created_at dipertahankan, updated_at=now (agar ikut sync). Daftarkan identitas grup baru → cegah duplikat internal antar-grup di file yang sama. Return {total, added, skipped, skippedList}. TIDAK menyentuh transactions/settings/printer. putProduct per item + notifyChange().
- `src/api.ts`: tambah wrapper `safeImportProducts`.
- `app/backup.tsx`: section BARU "RESTORE AMAN (TAMBAH DATA SAJA)" + tombol `backup-safe-restore` (pink outline). Alur: DocumentPicker → Alert konfirmasi PERSIS ("Restore data produk?" / "Produk yang sudah memiliki nama atau barcode yang sama akan dilewati dan tidak akan ditimpa." / [Batal] [Ya, Restore]) → jalankan → reload → Modal ringkasan (Berhasil ditambahkan/Dilewati/Total + daftar dilewati dgn pill alasan). testID: safe-added/safe-skipped/safe-total/safe-summary-close.
- DIVERIFIKASI: (a) logika inti diuji node vs seed asli 375 produk + file uji 7 produk → total 7, added 3 (1 baru + induk+anak), skipped 4 (Nama sama x2 incl case/spasi, Barcode sama via barcodes[], duplikat internal), finalCount=378 (data lama utuh, hanya +3), reason benar. (b) screenshot web: tombol "Restore Aman dari File" tampil rapi di halaman Backup, terpisah dari restore replace. Lint bersih. CATATAN: Alert konfirmasi + Modal ringkasan native-only untuk uji penuh → user REDEPLOY & uji di HP. Frontend-only.
...
## Session Log (fork) — FITUR Cari Lewat Suara (offline) di Cek Harga
- Library: `expo-speech-recognition@56.0.1` (mesin STT bawaan HP, on-device). Config plugin di app.json: microphonePermission/speechRecognitionPermission + androidSpeechServicePackages [googlequicksearchbox, as]. Android perms tambah RECORD_AUDIO; iOS infoPlist tambah NSMicrophoneUsageDescription + NSSpeechRecognitionUsageDescription.
- BARU `src/useVoiceSearch.ts`: hook. Load modul aman (web/Expo Go → available=false, TIDAK crash). start(): requestPermissionsAsync → start({lang:"id-ID", continuous:false, requiresOnDeviceRecognition:true}) → auto-stop saat selesai bicara. Event result(final)→onResult; error→map (NO_ID_PACK/PERM_BLOCKED/NO_SPEECH...). downloadIdPack()=androidTriggerOfflineModelDownload id-ID.
- BARU `src/fuzzySearch.ts`: fuzzyMatchProducts (normalisasi + Levenshtein per-token + bonus prefix/substring). FUZZY_THRESHOLD=0.6. DITEST node: rediona→Rexona(0.71), royko→Royco(0.80), sedap→Sedaap(0.83), abc, indomi goreng — semua PASS.
- `app/(tabs)/cek-harga.tsx`: tombol mic (testID cekharga-mic-btn) di searchRow; state voiceHint; doVoiceSearch(text)→fuzzy: skor≥0.6 → pickProduct(best) (auto buka + suara Miko); else "tidak ditemukan"+teks terdengar (OPSI A, reuse alur noResultQuery). Hint UI: listening banner, pesan error + tombol "Buka Pengaturan"(Linking.openSettings) / "Unduh paket bahasa". HANYA Cek Harga; tak sentuh transaksi/scanner/keranjang/produk/harga/stok.
- DIVERIFIKASI web (screenshot): mic tampil, tap→fallback "perlu di-build" (no crash). Fuzzy PASS via node. STT asli HANYA di build APK (native) → user WAJIB deploy ulang + BUILD BARU (modul native baru). Offline bergantung paket bahasa id di HP.
...
## Session Log (fork) — 5 fitur: −0,25, Ringkasan Harian, Stok Menipis (ambang), Struk WhatsApp, Edit Transaksi = alur normal
- Tombol −0,25 di Transaksi (`app/(tabs)/index.tsx`): pill `cart-minus025-{key}` di kiri `+0,25`, `cart.setQty(key, max(0.25, qty-0.25))` (clamp min 0,25). Style quarterBtnMinus/quarterTxtMinus.
- Ringkasan Harian (`app/(tabs)/riwayat.tsx`): kartu `riwayat-top-items` "Terlaris <periode>" (maks 3, berdasarkan JUMLAH UNIT terjual) dihitung dari `filtered` (ikut filter periode). Rows riwayat-top-0/1/2.
- Stok Menipis: Settings baru `lowStockThreshold` (default 5) di types.ts + DEFAULT_SETTINGS. Hook `src/useLowStockThreshold.ts` (reaktif via onLocalChange). Pengaturan Suara (`app/pengaturan-suara.tsx`): stepper `lowstock-value/inc/dec` (muncul saat Unlimited OFF). Produk (`app/(tabs)/produk.tsx`): chip `produk-lowstock-filter` (badge lowCount) filter produk non-variasi stok<=ambang; ProdukRow terima prop `threshold`. index.tsx cart low-warn pakai lowThreshold.
- Struk WhatsApp: `src/receipt.ts` `buildReceiptWhatsApp(tx,s)` (teks polos, tanpa ESC/POS). Komponen `components/WhatsAppReceiptButton.tsx` (tombol hijau + modal input nomor, normalisasi 08→62, buka wa.me via Linking). Dipasang di checkout done (`receipt-whatsapp`) & Riwayat detail (`riwayat-whatsapp`). Deep-link native-only.
- EDIT TRANSAKSI = alur transaksi normal (pilihan user: A/perbarui): `src/cart.tsx` tambah `editTxId/editCreatedAt` + `loadForEdit(txId,createdAt,items)` (bangun CartLine dari TxItem, tiers:[] agar harga terkunci; key produk::variasi agar scan ulang menambah qty). `clear()` reset mode edit. Riwayat `riwayat-edit` → `cart.loadForEdit(...)` + `router.navigate("/")` (tidak lagi buka /edit-transaksi). index.tsx banner `transaksi-edit-banner` + batal `transaksi-edit-cancel`; label bayar → "Simpan" saat editTxId. checkout `confirmPay`: bila editTxId → `api.updateTransaction` (ID & tanggal asli tetap, stok direkonsiliasi), else createTransaction. `/edit-transaksi` kini route yatim (dibiarkan, harmless).
- FIX KRITIS (dari testing agent iter17): urutan Provider di `app/_layout.tsx` diperbaiki → `ToastProvider` kini MEMBUNGKUS `BottomSheetModalProvider` (WhatsAppReceiptButton di dalam BottomSheet portal butuh ToastContext). Sebelumnya buka detail Riwayat red-screen "useToast must be used within ToastProvider".
- DIVERIFIKASI testing_agent iteration_17: 5/5 fitur PASS end-to-end di web; Edit menyimpan sbg UPDATE (tx count 103→103, id asli tetap). Native-only (printer/TTS/WA deep-link/HID) tak diuji. Lint bersih. Frontend-only → user REDEPLOY.


## Session Log (fork) — Fitur "Cetak Struk dari Gambar" (Bukti Pembayaran via OCR)
Keputusan user: OCR Hybrid (AI Vision online → fallback offline), model Gemini 3 Flash, simpan Gambar+PDF, DICATAT TERPISAH di Riwayat (tidak masuk omzet), Android only, printer 58mm, judul cetak "BUKTI TRANSAKSI TOKO BAGUS".

- BACKEND (`server.py`): endpoint `POST /api/ocr/bukti` {image_base64, mime_type} → Gemini vision (`gemini-3-flash-preview` via emergentintegrations + EMERGENT_LLM_KEY; fallback openai_compatible via LLM_* env). Sistem prompt Indonesia "jangan menebak": field ragu → value null + confident false. Respons {fields:{method,recipient,amount,date,time,ref}} tiap {value,confident}; amount dipaksa integer. `_parse_ocr_json`/`_norm_field` robust. Sync: koleksi baru `s_bukti` di pull/push + index startup (store,id)/(store,updated_at).
- FRONTEND:
  - types.ts: type `Bukti` {id,method,recipient,amount,date,time,ref,customer,image_uri?,created_at,updated_at}.
  - localdb.ts: tabel SQLite `bukti` + in-memory `buktiList` + load/persist + `getBukti/saveBukti/deleteBukti`; collectDirty/applyRemote menyertakan `bukti` (image_uri TIDAK disinkron). sync.ts push chunk bukti.
  - api.ts: `ocrBukti(base64,mime)` (fetch ke /api/ocr/bukti), `getBukti/saveBukti/deleteBukti`, type OcrBuktiResult.
  - receipt.ts: `buildBuktiReceiptText` (ESC/POS 58mm, judul "BUKTI TRANSAKSI TOKO BAGUS" + identitas toko + fields + "Salinan bukti pembayaran") & `buildBuktiReceiptHTML` (PDF). Tambah C_BOLD.
  - Layar `app/baca-bukti.tsx`: pilih dari galeri (expo-image-picker base64) ATAU via Share (baca file → base64 pakai expo-file-system/legacy) → OCR → form editable, field ragu ditandai oranye "Periksa". Aksi: Cetak (Bluetooth 58mm), Bagikan (PDF via expo-print+expo-sharing), Simpan Gambar (view-shot→expo-media-library), Simpan PDF, "Simpan & Kembali ke Transaksi". Idempoten simpan ke Riwayat (savedId ref). Prefill via params untuk cetak ulang.
  - Riwayat: ikon kamera `riwayat-bukti-scan` → /baca-bukti; chip toggle `riwayat-filter-bukti` ("Bukti Bayar") menampilkan daftar bukti (badge "BUKTI") disaring periode; omzet/terlaris TIDAK terpengaruh (tetap dari transaksi). Tap kartu → cetak ulang (prefill), long-press → hapus.
  - Tahap 5 Share Target: `expo-share-intent@5.1.1` (SDK54) + plugin app.json (androidIntentFilters image/*) + `components/ShareIntentHandler.tsx` (useShareIntent → router.push /baca-bukti). HANYA aktif di build Android; aman/no-op di web preview (sudah diverifikasi app tetap boot).
  - Deps ditambah: expo-image-picker, expo-media-library, expo-print, expo-share-intent@5.1.1. app.json: izin READ_MEDIA_IMAGES + iOS photo usage strings + plugin image-picker/media-library.
- DIVERIFIKASI testing_agent iteration_18: BACKEND 5/5 (OCR happy-path amount=75000, "jangan menebak" null+confident false, empty→400, sync bukti round-trip) + FRONTEND 5/5 alur (buka layar, isi manual, catat, tampil badge, omzet tidak berubah, reprint prefill). Native-only (cetak/share/save/pick/OCR-galeri/Share-Target) hanya di build APK. Lint bersih.
- CATATAN: `expo-file-system/legacy` dipakai untuk baca file share (deprecation non-blocking di SDK54). image_testing.md disimpan di /app.
