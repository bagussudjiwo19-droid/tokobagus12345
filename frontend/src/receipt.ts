...
// Format nomor token listrik agar mudah dibaca: kelompok 4 digit → "1234 5678 ...".
...
export function buildBuktiReceiptText(b: Bukti, s: Settings): string {
  if (b.kind === "token") return buildTokenReceiptText(b, s);
  if (b.kind === "pulsa") return buildPulsaReceiptText(b, s);
  return buildTransferReceiptText(b, s);
}

// -------- Kepala struk umum (nama toko + alamat + telepon), rata tengah. --------
function receiptHead(s: Settings, title: string): string[] {
  const L: string[] = [];
  const shop = (s.showShopName && s.shopName ? s.shopName : "TOKO BAGUS").toUpperCase();
  L.push(C_CENTER + C_BIG_BOLD + center(shop).trimStart() + C_NORMAL + C_LEFT);
  if (s.showAddress && s.address) wrapWords(s.address, CW).forEach((l) => L.push(center(l)));
  if (s.showPhone && s.phone) L.push(center(s.phone));
  L.push(LINE);
  L.push(C_BOLD + center(title).trimStart() + C_NORMAL);
  L.push(LINE);
  return L;
}

function receiptFoot(s: Settings): string[] {
  const L: string[] = [];
  if (s.showNote && s.note) L.push(center(s.note));
  L.push(C_CENTER + center(s.showThanks && s.thanks ? s.thanks : "TERIMA KASIH").trimStart() + C_LEFT);
  return L;
}

// -------- STRUK TOKEN LISTRIK (58mm) --------
export function buildTokenReceiptText(b: Bukti, s: Settings): string {
  const L: string[] = [...receiptHead(s, "TOKEN LISTRIK / PLN")];
  L.push(twoCols("STATUS", (b.status || "BERHASIL")));
  if (s.showDateTime) L.push(twoCols("Waktu", [b.date, b.time].filter(Boolean).join(" ") || receiptDateTime(b.created_at)));
  L.push(LINE);
  L.push(...labeledWrapped("NAMA PELANGGAN :", b.customer_name || "-"));
  L.push(...labeledWrapped("ID PELANGGAN / NO. METER :", b.customer_id || "-"));
  L.push(LINE);
  L.push(line("NOMOR TOKEN LISTRIK"));
  const tok = groupToken(b.token || "");
  if (tok) wrapWords(tok, CW).forEach((l) => L.push(C_CENTER + C_BIG_BOLD + center(l).trimStart() + C_NORMAL + C_LEFT));
  else L.push(center("-"));
  L.push(LINE);
  L.push(twoCols("Nominal", "Rp" + numberID(b.amount || 0)));
  if (b.sell_price) L.push(C_BOLD + twoCols("Harga Jual", "Rp" + numberID(b.sell_price)) + C_NORMAL);
  L.push(LINE);
  L.push(...receiptFoot(s));
  return L.join("\n") + "\n\n\n";
}

// -------- STRUK PULSA HP (58mm) --------
export function buildPulsaReceiptText(b: Bukti, s: Settings): string {
  const L: string[] = [...receiptHead(s, "PULSA HP")];
  L.push(twoCols("STATUS", (b.status || "BERHASIL")));
  if (s.showDateTime) L.push(twoCols("Waktu", [b.date, b.time].filter(Boolean).join(" ") || receiptDateTime(b.created_at)));
  L.push(LINE);
  L.push(twoCols("No. HP", b.phone || "-"));
  L.push(twoCols("Operator", b.operator || "-"));
  L.push(LINE);
  L.push(line("NOMINAL PULSA"));
...
export function buildTransferReceiptText(b: Bukti, s: Settings): string {
  const L: string[] = [];
...
export function buildBuktiReceiptHTML(b: Bukti, s: Settings): string {
  if (b.kind === "token") return buildTokenReceiptHTML(b, s);
  if (b.kind === "pulsa") return buildPulsaReceiptHTML(b, s);
  return buildTransferReceiptHTML(b, s);
}

const escHtml = (t: string) => (t || "").replace(/[&<>]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" }[c] as string));

function htmlShell(s: Settings, title: string, inner: string): string {
  const shop = escHtml((s.showShopName && s.shopName ? s.shopName : "TOKO BAGUS").toUpperCase());
  const addr = s.showAddress && s.address ? `<div class="s">${escHtml(s.address)}</div>` : "";
  const phone = s.showPhone && s.phone ? `<div class="s">${escHtml(s.phone)}</div>` : "";
  const thanks = escHtml(s.showThanks && s.thanks ? s.thanks : "TERIMA KASIH");
...
<style>
  ...
  * { font-family: 'Courier New', monospace; }
  ...
  .sub { font-weight: 700; font-size: 13px; text-align:center; margin-top:2px; }
  ...
  .rowb .v { font-weight: 700; }
  .lbl { font-weight: 700; font-size: 12px; }
  .amt { text-align:center; font-weight: 700; font-size: 18px; margin: 4px 0; }
  .token { text-align:center; font-weight: 700; font-size: 17px; letter-spacing: 1px; word-break: break-word; margin: 2px 0; }
</style></head><body>
  ...
  ${inner}
  <hr/>
  <div class="s">${thanks}</div>
</body></html>`;
}

export function buildTokenReceiptHTML(b: Bukti, s: Settings): string {
  const row = (k: string, v: string, bold = false) => `<div class="row ${bold ? "rowb" : ""}"><span>${escHtml(k)}</span><span class="v">${escHtml(v || "-")}</span></div>`;
  const inner = `
  <div class="row"><span>STATUS</span><span class="v">${escHtml(b.status || "BERHASIL")}</span></div>
  ${row("WAKTU", [b.date, b.time].filter(Boolean).join(" ") || receiptDateTime(b.created_at))}
  <hr/>
  ${row("NAMA PELANGGAN", b.customer_name || "-")}
  ${row("ID/NO. METER", b.customer_id || "-")}
  <hr/>
  <div class="lbl">NOMOR TOKEN LISTRIK</div>
  <div class="token">${escHtml(groupToken(b.token || "") || "-")}</div>
  <hr/>
  ${row("NOMINAL", "Rp" + numberID(b.amount || 0))}
  ${b.sell_price ? row("HARGA JUAL", "Rp" + numberID(b.sell_price), true) : ""}`;
  return htmlShell(s, "TOKEN LISTRIK / PLN", inner);
}

export function buildPulsaReceiptHTML(b: Bukti, s: Settings): string {
  const row = (k: string, v: string, bold = false) => `<div class="row ${bold ? "rowb" : ""}"><span>${escHtml(k)}</span><span class="v">${escHtml(v || "-")}</span></div>`;
  const inner = `
  <div class="row"><span>STATUS</span><span class="v">${escHtml(b.status || "BERHASIL")}</span></div>
  ${row("WAKTU", [b.date, b.time].filter(Boolean).join(" ") || receiptDateTime(b.created_at))}
  ...
  <hr/>
  <div class="lbl">NOMINAL PULSA</div>
  <div class="amt">Rp${numberID(b.amount || 0)}</div>
  ${b.sell_price ? row("HARGA JUAL", "Rp" + numberID(b.sell_price), true) : ""}`;
  return htmlShell(s, "PULSA HP", inner);
}

export function buildTransferReceiptHTML(b: Bukti, s: Settings): string {
