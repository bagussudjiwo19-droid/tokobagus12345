...
export type BuktiKind = "transfer" | "token" | "pulsa";

export type Bukti = {
  id: string;
  kind?: BuktiKind;          // jenis transaksi: transfer (default) / token listrik / pulsa HP
  status: string;            // internal: BERHASIL / TERVERIFIKASI / PENDING / GAGAL
  amount: number;            // nominal transaksi (transfer/token/pulsa) — rupiah, bulat
  sell_price?: number;       // Harga jual ke pelanggan (Token/Pulsa)
  // -------- Token Listrik --------
  customer_name?: string;    // Nama pelanggan
  customer_id?: string;      // ID pelanggan / nomor meter
  token?: string;            // Nomor token listrik (stroom)
  // -------- Pulsa HP --------
  phone?: string;            // Nomor HP
  operator?: string;         // Operator (Telkomsel, XL, Indosat, dll)
  // -------- Transfer (existing) --------
