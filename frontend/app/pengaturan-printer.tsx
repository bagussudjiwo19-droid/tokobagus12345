...
export default function PengaturanPrinterScreen() {
  ...
  const choose = async (d: BTDevice) => {
    try { await api.savePrinter({ address: d.address, name: d.name }); setPrinter({ address: d.address, name: d.name }); toast.show(`Printer ${d.name} dipilih`, "success"); }
    catch { toast.show("Gagal menyimpan printer", "error"); }
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: insets.bottom + 40 }}>
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoTxt}>Nama toko & isi struk kini diatur di menu Pengaturan Struk. Struk mencetak sebagai "TOKO BAGUS".</Text>
        </View>

        <Pressable style={styles.outlineBtn} onPress={scan} testID="printer-scan">
        ...
        <Text style={styles.subtext}>Pasangkan printer lewat pengaturan Bluetooth HP dulu, lalu tekan Cari.</Text>

        {devices.map((d) => (
          <Pressable key={d.address} style={styles.deviceRow} onPress={() => choose(d)} testID={`printer-device-${d.address}`}>
            <Ionicons name="bluetooth" size={20} color={colors.brand} />
            <Text style={styles.deviceName}>{d.name}</Text>
            {printer.address === d.address && <Ionicons name="checkmark" size={20} color={colors.success} />}
          </Pressable>
        ))}
