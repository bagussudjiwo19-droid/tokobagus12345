...
export default function RiwayatScreen() {
  ...
  const openBukti = (b: Bukti) => {
    router.push({ pathname: "/baca-bukti", params: {
      id: b.id, kind: b.kind || "transfer", status: b.status, amount: String(b.amount),
      sell_price: String(b.sell_price || ""),
      customer_name: b.customer_name || "", customer_id: b.customer_id || "", token: b.token || "",
      phone: b.phone || "", operator: b.operator || "",
  ...
  const renderBukti = ({ item }: { item: Bukti }) => {
    const kind = item.kind || "transfer";
    const meta = kind === "token"
      ? { title: item.customer_name ? `Token · ${item.customer_name}` : "Token Listrik", badge: "TOKEN", icon: "flash-outline" as const, sub: item.customer_id ? `No. Meter ${item.customer_id}` : "Token Listrik / PLN" }
      : kind === "pulsa"
        ? { title: item.phone ? `Pulsa · ${item.phone}` : "Pulsa HP", badge: "PULSA", icon: "phone-portrait-outline" as const, sub: item.operator || "Pulsa HP" }
        : { title: item.method || "Pembayaran", badge: "BUKTI", icon: "card-outline" as const, sub: item.sender_name ? `${item.sender_name}` : (item.recipient || "Transfer") };
    return (
      <Pressable style={styles.buktiRow} onPress={() => openBukti(item)} onLongPress={() => removeBukti(item)} testID={`riwayat-bukti-${item.id}`}>
        <View style={styles.buktiThumb}><Ionicons name={meta.icon} size={20} color={colors.brand} /></View>
        <View style={{ flex: 1 }}>
          <View style={styles.rowTopLine}>
            <Text style={styles.rowDate} numberOfLines={1}>{meta.title}</Text>
            <View style={styles.buktiBadge}><Text style={styles.buktiBadgeTxt}>{meta.badge}</Text></View>
          </View>
          <Text style={styles.rowSub} numberOfLines={1}>
            {formatDateID(item.created_at)} · {meta.sub}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  buktiHint: { flexDirection: "row", alignItems: "center", gap: 6, marginHorizontal: spacing.lg, marginBottom: spacing.sm },
  buktiHintTxt: { color: colors.muted, fontFamily: font.medium, fontSize: fontSize.sm },
  buktiRow: { flexDirection: "row", alignItems: "center", gap: spacing.sm, backgroundColor: colors.surfaceSecondary, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, paddingVertical: spacing.sm, paddingHorizontal: spacing.md },
  buktiThumb: { width: 38, height: 38, borderRadius: 12, backgroundColor: colors.brandTertiary, alignItems: "center", justifyContent: "center" },
  buktiBadge: { backgroundColor: colors.brand, borderRadius: radius.sm, paddingHorizontal: spacing.sm, paddingVertical: 2 },
  buktiBadgeTxt: { color: colors.onBrandPrimary, fontFamily: font.bold, fontSize: 10, letterSpacing: 0.5 },
  buktiHintSmall: { color: colors.muted, fontFamily: font.regular, fontSize: fontSize.xs, marginTop: 1 },
